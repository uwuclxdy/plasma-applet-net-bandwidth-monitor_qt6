...
import QtQuick.Layouts  // # CUSTOM COLOR PICKER ##
...
Item {
    ...
    NumberAnimation {
        ...
        onStopped: {  
            ...
            var buttonObject = target.children[0].children[0]               // GET infoButton Object
    ...
    OpacityAnimator {
        ...
        onStopped: {   
            ...
            var buttonObject = target.parent.parent.children[0].children[0] // GET infoButton Object (MouseArea for the state)
            var buttonState = buttonObject.state                            // GET infoButton state
    ...
    function toggleTooltip(rec) {           // SEND ENTIRE ROW OBJECT TO ANIMATION FUNCTION
        ...
        var rec_state       = rec_infoButton.children[0].state      // GET rec infoButton MouseArea state toggle
        
        
        console.log("rec_infoText:", rec_infoText)
        
        console.log("rec_state:", rec_state)
    ...
    function getToolTipText(zone) {
        ...
        if (zone === 0) {
            text = "<table>"
                        ...
                        + "<tr><td></td> <td><img src='../ui/image/1a2.png' width='120' height='65'></td></tr>"
                        + "<tr><td><b>" + i18n("Horizontal") + "</b>: </td> <td>" + i18n("Upload and Download will be aligned side by side:") + "</td></tr>"
                        + "<tr><td></td> <td><img src='../ui/image/1b2.png' width='240' height='40'></td></tr>"
                    + "</table>"
        } else if (zone === 2) {
            text = "<table>"
                        ...
                        + "<tr><td></td> <td><img src='../ui/image/2a.png' width='150' height='65'><img src='../ui/image/2b.png' width='250' height='70'></td></tr>"
                    + "</table>"
        } else if (zone === 3) {
            text = "<table>"
                        ...
                        + "<tr><td></td> <td><img src='../ui/image/3a.png' width='305' height='65'></td></tr>"
                    + "</table>"
        } else if (zone === 4) {
            text = "<table>"
...
Using the per second prefix ('/s' or 'ps') with this option is not technically correct but the choice is yours.")
            ...
            text = "<table>"
                        ...
                        + "<tr><td></td> <td style='padding-bottom:6px'><img src='../ui/image/4a.png' width='280' height='85'></td></tr>"
                    + "</table>"
...
The prefixes 'kibi,' 'mebi,' 'gibi,' etc. have been introduced to refer to binary 1024 units, while the prefixes 'kilo,' 'mega,' 'giga,' etc. continue to refer to decimal 1000 units.")
    ...
    ScrollView {
        id: scrollView
        anchors.fill: main
        contentHeight: 1400 // PUT ALL CONTENT IN A SEPERATE RECTANGLE - NEXT RELEASE
    ...
    //#######################################################################################################
        Rectangle {
            ...
            ToolButton {
                id: infoButton175
                icon.name: 'kt-info-widget'
                implicitWidth: infoButton175.height
                anchors.right: parent.right
                opacity: 0.5
                MouseArea {
                    property bool state:    false
                    anchors.fill:           parent
                    hoverEnabled:           true
                    onEntered:              infoButton175.opacity = 1
                    onExited:               state ? infoButton175.opacity = 1 : infoButton175.opacity = 0.5
                    onClicked: {
                        state = !state
                        main.toggleTooltip(rootRec175)
                    }
                }
            }
            Grid {
            ...
            Grid {
                // NEW TOOLTIP INFORMATION
                anchors.top: gA175.bottom
                Text {
                    id: infoText175
                    opacity: 0
                    padding: 10 
                    width: rootRec175.width
                    color: myPalette.windowText
                    textFormat: Text.RichText
                    wrapMode: Text.Wrap  
                    text: getToolTipText(175)
                }
            }  
    ...
    //#######################################################################################################
        Rectangle {
            ...
            ToolButton {
                id: infoButton185
                icon.name: 'kt-info-widget'
                implicitWidth: infoButton185.height
                anchors.right: parent.right
                opacity: 0.5
                MouseArea {
                    property bool state:    false
                    anchors.fill:           parent
                    hoverEnabled:           true
                    onEntered:              infoButton185.opacity = 1
                    onExited:               state ? infoButton185.opacity = 1 : infoButton185.opacity = 0.5
                    onClicked: {
                        state = !state
                        main.toggleTooltip(rootRec185)
                    }
                }
            }
            Grid {
            ...
            Grid {
                // NEW TOOLTIP INFORMATION
                anchors.top: gA185.bottom
                Text {
                    id: infoText185
                    opacity: 0
                    padding: 10 
                    width: rootRec185.width
                    color: myPalette.windowText
                    textFormat: Text.RichText
                    wrapMode: Text.Wrap  
                    text: getToolTipText(185)
                }
            }  
                ...
                ColorDialog {
                    ...
                    onAccepted: {
                        if (colorDialog.selectedColor) {
                            ...
                            colorTextField1.text = colorDialog.selectedColor // Set the hex value in the text field
                            colorChoiceBG1.color = colorDialog.selectedColor  
                        }   
                    }
                }
                Popup {
                    ...
                    Rectangle {
                        ...
                        RowLayout {
                            ...
                            Rectangle {
                                ...
                                width: colorPickerPopup1.width - okButton1.width - colorDialogButton1.width - colorChoiceDefault1.width - 44 // 22 //colorChoiceField.width
    ...
    //#######################################################################################################
        Rectangle {
            ...
            Grid {
                ...
                ColorDialog {
                    id: speedColorDialog
                    title: "Choose a color: Speed based colour" // + gA23.selectedSpeedColorButton.text - square is sent to the var, square doesnt contain text.
                    onAccepted: {
                        if (speedColorDialog.selectedColor) {
                            colorTextField.text = speedColorDialog.selectedColor // Set the hex value in the text field
                            colorChoiceBG.color = speedColorDialog.selectedColor    
                        }          
                    }
                }  
                // CUSTOM COLOR PICKER
                Popup {
                    ...
                    width: 428 
                    // {
                    //     console.log("colorGrid.width: " + colorGrid.width, "colorPickerPopup: " + colorPickerPopup.width)
                    //     return 428 //(gA23.labelSpeedColorPadding * 2) // 400 +8
                    // }
                    height: 264
                    //modal: true
                    // padding: labelSpeedColorPadding
                    parent: Overlay.overlay
                    x: Math.round((parent.width - width) / 2)       // center color popup on x axis
                    y: Math.round((parent.height - height) / 2)     // center color popup on y axis
                    ...
                    Rectangle {
                        ...
                        RowLayout {
                            ...
                            Rectangle {
                                id: colorChoiceBG
                                        // window width - 3 buttons
                                width: colorPickerPopup.width - okButton.width - colorDialogButton.width - colorChoiceDefault.width - 44 //colorChoiceField.width
                    ...
                    Column {
                        ...
                        RowLayout {
                            ...
                            Rectangle {
                                ...
                                Layout.preferredWidth: infoButton23.height / 2
                                Layout.preferredHeight: infoButton23.height / 2
                                radius: 4
                                MouseArea {
                                    anchors.fill: parent
                                    hoverEnabled: true
